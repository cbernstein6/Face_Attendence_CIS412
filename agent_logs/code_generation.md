...
Prompt: "Write a Python function enroll_user(name, email, image_path) for enroll.py. Use the face_recognition library to validate the image contains exactly one face. Reject images with zero or multiple faces. Copy valid images into data/faces/ with a unique filename, then call add_user_to_csv from utils. Return a success tuple." 
...
Prompt: "Write main.py with a menu loop offering: enroll, create event, check-in to event, quick check-in, list events, exit. Wire each option to the existing modules (enroll, recognize, attendance, events). Use plain input() prompts." 
